<?php
	...
	if(!empty($_FILES['coffeePhoto']['name']))
	{
		...
		$newPhotoName = strtolower($randomDigit . "_" . $coffeePhoto); //attach the random digit to the front of uploaded image's name
		$target = "../images/" . $newPhotoName; //the target location for uploaded images

		$allowedExts = array('jpg', 'jpeg', 'gif', 'png'); //allowed file extensions
		$tmp = explode('.', $_FILES['coffeePhoto']['name']); //split the file name from the file extension
		$extension = end($tmp); //retrieve the extension of the photo e.g., jpg or png
		...
		//check that only accepted image formats can be uploaded
